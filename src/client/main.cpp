...
int main(int argc,char **argv){
    //判断命令个数
    if(argc<3){
        cerr<<"command invalid! example: ./ChatClient 127.0.0.0 6000"<<endl;
        exit(-1);
    }
    //解析通过命令行形参传递的ip和port
    char *ip=argv[1];
...
void creategroup(int clientfd,string str){
    int idx=str.find(":");
    if(-1 == idx){
        cerr<<" creategroup cammand invalid"<<endl;
        return;
    }
